...
result.getRoot();                    // the differences as a tree of schema locations
```

The differences themselves changed shape too:

- **A nested change is reported where it is.** Tightening `maxLength` on property `a` used to
  report `OBJECT_TYPE_PROPERTY_SCHEMAS_CHANGED` at `/a`; it now reports
  `STRING_TYPE_MAX_LENGTH_DECREASED` at `/properties/a/maxLength`. The container types
  (`OBJECT_TYPE_PROPERTY_SCHEMAS_CHANGED`, `ARRAY_TYPE_ITEM_SCHEMAS_CHANGED`, and similar) remain
  only for a nested schema replaced as a whole, e.g. with `false`. Compatible nested changes,
  which were not reported at all, are reported too. Code matching on the container types should
  match on the leaf types or on the path instead.
- **Paths are `JsonPointer`s.** `Difference.getPathOriginal()` and `getPathUpdated()` return a
  `JsonPointer` ending in the keyword that changed, rather than a `String` without keywords
  (`/a`); call `toString()` for the string form. `getPathOriginal()` is now filled in, and each
  path names keywords as its own schema does (`/items/0` for a draft-7 tuple, `/prefixItems/0`
  for 2020-12). A member added to or removed from a list or map, such as `required`, points at
  the member (`/required/1`).
- **`getSubSchemaOriginal()` / `getSubSchemaUpdated()` are gone.** They held `toString()` of
  whatever was compared — a number for a bound, an object identity string such as
  `JCFullSchemaImpl@4e5ed836` for a schema. The paths lead to the same place in each schema.
...
!!! warning "Check `hasUnsupportedFeatures()`"
    ...
    The list is populated only when a dereferencer is configured, and holds the references it
    could not resolve — the sub-schemas behind those were never compared. See
    [Schema Compatibility](../user-guide/schema-compatibility.md#unsupported-features).
...
`Library` gained methods for working with documents whose root need not be an object — a JSON
Schema root may be the literal `true` or `false`:

```java
RootCapable root = Library.createRoot(modelType);
RootCapable root = Library.readRoot(jsonNode);
RootCapable root = Library.readRootFromJSONString(jsonString);
JsonNode json = Library.writeRoot(root);
String jsonString = Library.writeRootToJSONString(root);
```

A boolean root is a `RootCapable` but not a `Node`, so `writeNode` cannot take it; `writeRoot`
writes any root. JSON Schema reference resolution returns `Optional<JsonSchema>` rather than
`Optional<Node>` for the same reason: `FragmentResolver`, `ResourceResolver`,
`JsonSchemaRefResolver` and `JsonSchemaRefTraversal.resolveRef` changed accordingly, so a custom
`ResourceResolver` returns `Optional.of((JsonSchema) root)` instead of a `Node`.
...
| 3.x — `…models.union` | 4.0 |
|---|---|
| `MultiFormatSchemaSchemaUnion` | `…models.asyncapi` |
| `AnySchemaUnion` | `…models` |
| `BooleanSchemaUnion` | `…models` |
| `SchemaSchemaListUnion` | `…models` |
| `SchemaListUnionValue`, `…Impl` | `…models` |
| `ObjectUnionValue`, `…Impl` | renamed `AnyUnionValue`, `…Impl` (same package) |
...
| 3.x | 4.0 |
|---|---|
| `BooleanJSchemaJSchemaListUnion` | `…models.jsonschema.BooleanFullSchemaJsonSchemaListUnion` |
| `JSchemaListUnionValue`, `…Impl` | `…models.jsonschema.JsonSchemaListUnionValue`, `…Impl` |
...
The tuple form of `items` (drafts 4 to 2019-09) is the exception: its elements may be boolean
schemas, as the specifications allow, so the list is a list of `JsonSchema` rather than of full
schemas. Its accessors are `isJsonSchemaList()` / `asJsonSchemaList()`, returning
`List<JsonSchema>`; check each element with `isFullSchema()` / `isBoolean()` before using it.
